...
Similarly to <<../../inputs/input-fields#,input fields>>, the focus ring is only rendered when the button is focused by keyboard or programmatically.
...
In lists of selectable items (such as in a <<../../data/grid#,Grid>>) that provide actions applicable to the selected item, buttons for selection-specific actions should be placed separately from "global" non-selection-specific actions, preferably below the list of selectable items.
